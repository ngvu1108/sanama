...
spawn(function()
    while task.wait() do
        ...
        elseif BringMobs and AutoTyrant then
            pcall(function()
                BringMonster(Tyrant_Farm_Name, Tyrant_Farm_CFrame)
            end)
        end
    end
end)
...
DisSeaFarm = DisSeaFarm or 40 -- Increased default for higher flight
...
AutoLeviathan = AutoLeviathan or false -- New for Auto Leviathan
AutoHarpoon = AutoHarpoon or false -- Auto toggle for harpoon phase
SelectReturnIsland = SelectReturnIsland or "Tiki" -- Default return
_G.SeaSkillZ = _G.SeaSkillZ or false
_G.SeaSkillX = _G.SeaSkillX or false
_G.SeaSkillC = _G.SeaSkillC or false
_G.SeaSkillV = _G.SeaSkillV or false
_G.SeaSkillF = _G.SeaSkillF or false
BuyingBoat = false -- Flag to prevent spamming buys
LastBuyTime = 0 -- Timestamp to add a cooldown (e.g., 5 seconds)
...
local FactionBoats = {"Sloop", "Brigade", "GrandBrigade"} -- Boats that need faction prefix
local SelectBoat = BoatList[1]

--// Get full boat name based on faction
local function getFullBoatName()
    local prefix = ""
    local team = game.Players.LocalPlayer.Team
    if team then
        local faction = (team.Name == "Marines" and "Marine" or (team.Name == "Pirates" and "Pirate" or ""))
        if table.find(FactionBoats, SelectBoat) then
            prefix = faction
        end
    end
    return prefix .. SelectBoat
end
...
local RoughSeaPos = CFrame.new(-47041.6641, 20, -6858.74072, 0.703071356, -0.0286799595, 0.710540712, 0.0407584943, 0.999169052, -2.41380471e-09, -0.709950268, 0.0289605707, 0.703656077) -- Increased y to 20 for higher flight
local HydraPos = CFrame.new(-1576, 7, -2983, -0.515, 0, -0.857, 0, 1, 0, 0.857, 0, -0.515) -- Example Hydra pos, adjust if needed
local MysteriousScientistPos = CFrame.new(-16200, 10, 500) -- Approximate pos at Tiki, spy to confirm
local LeviathanHeartPos = nil -- Dynamic, detect later

--// TWEEN BOATS (uses selected boat safely, improved direction speed)
function TPB(BoatsPos)
    ...
    local fullBoatName = getFullBoatName()
    local boat = boatsFolder:FindFirstChild(fullBoatName)
    ...
    -- Improve direction: Calculate alignment, boost speed if straight
    local direction = (targetPos - seatPos).Unit
    local currentVel = seat.Velocity.Unit
    local alignment = direction:Dot(currentVel)
    if alignment > 0.9 then
        Speed = Speed * 1.5 -- Faster straight
    elseif math.abs(direction:Cross(currentVel).Magnitude) > 0.5 then
        Speed = Speed * 0.7 -- Slower on turns
    end
...
function StopBoats(target)
    if not target then
        ...
        local boatsFolder = game:GetService("Workspace"):FindFirstChild("Boats")
        local fullBoatName = getFullBoatName()
        if boatsFolder and boatsFolder:FindFirstChild(fullBoatName) and boatsFolder[fullBoatName]:FindFirstChild("VehicleSeat") then
            TPB(boatsFolder[fullBoatName].VehicleSeat.CFrame)
        end
        wait(.1)
        _G.StopTween = false
    else
...
--// Boat Tween Anchor (prevent collisions for selected boat when AutoSail)
spawn(function()
    pcall(function()
        game:GetService("RunService").Stepped:Connect(function()
            if AutoSail or AutoLeviathan then
                local boatsFolder = game:GetService("Workspace"):FindFirstChild("Boats")
                local fullBoatName = getFullBoatName()
                if boatsFolder and boatsFolder:FindFirstChild(fullBoatName) then
                    for _,v in pairs(boatsFolder[fullBoatName]:GetDescendants()) do
                        if v:IsA("BasePart") then
                            v.CanCollide = false
                        end
                    end
...
--// Boats Body Velocity (update BodyVelocity for selected boat, improved for direction)
spawn(function()
    while task.wait() do
        pcall(function()
            local boatsFolder = game:GetService("Workspace"):FindFirstChild("Boats")
            local fullBoatName = getFullBoatName()
            if boatsFolder and boatsFolder:FindFirstChild(fullBoatName) and boatsFolder[fullBoatName]:FindFirstChild("VehicleSeat") then
                local seat = boatsFolder[fullBoatName].VehicleSeat
                if AutoSail or AutoLeviathan then
                    if seat:FindFirstChild("BodyVelocity") then
                        seat.BodyVelocity.MaxForce = Vector3.new(100000,100000,100000)
                        seat.BodyVelocity.Velocity = Vector3.new(0,0,0) -- Keep 0 for tween control
                    end
                else
                    if seat:FindFirstChild("BodyVelocity") then
...
--// Fix Boat Sinking (keep float when player not sitting)
spawn(function()
    while task.wait(0.5) do
        if AutoSail or AutoLeviathan or AutoFarmTerrorShark or AutoFarmFish or AutoFarmSeaBeast or AutoFarmGhostBoats then
            pcall(function()
                local boatsFolder = game:GetService("Workspace"):FindFirstChild("Boats")
                local fullBoatName = getFullBoatName()
                local boat = boatsFolder and boatsFolder:FindFirstChild(fullBoatName)
                if boat and game.Players.LocalPlayer.Character.Humanoid.Sit == false then
                    for _, part in pairs(boat:GetDescendants()) do
                        if part:IsA("BasePart") then
                            part.CustomPhysicalProperties = PhysicalProperties.new(0.1, 0.3, 0.5, 0.3, 1) -- Low density for buoyancy/float
                        end
                    end
                    local seat = boat:FindFirstChild("VehicleSeat")
                    if seat and seat.Position.Y < 15 then -- If sinking below 15, reset y
...
--// Auto Rough Sea main loop (uses SelectBoat) with priority sequence
spawn(function()
    while task.wait() do
        if AutoSail then
            pcall(function()
                local boatsFolder = game:GetService("Workspace"):FindFirstChild("Boats")
                local fullBoatName = getFullBoatName()
                local selectedBoatExists = boatsFolder and boatsFolder:FindFirstChild(fullBoatName)
                if not selectedBoatExists and not BuyingBoat and (tick() - LastBuyTime > 5) then -- Added checks: not buying, and 5-sec cooldown
                    BuyingBoat = true -- Set flag to prevent repeats
                    LastBuyTime = tick() -- Update timestamp
                    ...
                    if hrp and (TikiPos.Position - hrp.Position).Magnitude <= 10 then
                        local prefix = ""
                        ...
                        wait(5) -- Increased wait for boat to spawn
                    end
                    BuyingBoat = false -- Reset flag after attempt
                else
                    local boat = boatsFolder[fullBoatName]
                    local seat = boat and boat:FindFirstChild("VehicleSeat")
                    if seat then
                        if game.Players.LocalPlayer.Character:WaitForChild("Humanoid").Sit == false then
                            Tween(seat.CFrame * CFrame.new(0,1,0))
                        elseif game.Players.LocalPlayer.Character:WaitForChild("Humanoid").Sit == true then
                            -- Check if any enemy is present - DON'T move boat if enemies exist
                            local hasEnemy = game.Workspace.Enemies:FindFirstChild("Terrorshark") or 
                                           ...
                                           game.Workspace.Enemies:FindFirstChild("PirateBrigade")
                            
                            if not hasEnemy then
                                -- Only move boat when no enemies are present
                                TPB(RoughSeaPos)
                                local CameraShakerR = require(game.ReplicatedStorage.Util.CameraShaker)
                                CameraShakerR:Stop()
                                ...
                                -- If enemies exist, stop boat movement and stay in position
                                StopBoats(seat.CFrame)
                            end
                        end
                    end
                end
                -- nếu player chết -> hủy tàu được chọn (nếu có)
                ...
                if playerHum and playerHum.Health <= 0 then
                    pcall(function()
                        local fullBoatName = getFullBoatName()
                        if game.Workspace:FindFirstChild("Boats") and game.Workspace.Boats:FindFirstChild(fullBoatName) then
                            game.Workspace.Boats[fullBoatName]:Destroy()
                        end
                    end)
                end
            end)
...
--// IMPROVED ATTACK SYSTEM FROM FIRST SCRIPT
SeaUseSkill = false
SeaMonName = ""
SeaMonCFrame = nil
SeaMonPosition = nil

-- Function to get tool name by type
local function get_tool_name_by_type(typ)
    for _, v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
        if v:IsA("Tool") and v.ToolTip == typ then
            return v.Name
        end
    end
...
--// Attack TerrorShark [Boss] - NORMAL ATTACKS ONLY
            ...
            pcall(function()
                if game.Workspace.Enemies:FindFirstChild("Terrorshark") then
                        ...
                        if v.Name == "Terrorshark" then
                            if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                                repeat game:GetService("RunService").Heartbeat:wait()
                                    -- NO SKILLS FOR TERRORSHARK - NORMAL ATTACKS ONLY
                    ...
                    if game.Players.LocalPlayer.Character:WaitForChild("Humanoid").Sit == false then
                        ...
                        local fullBoatName = getFullBoatName()
                        if boatsFolder and boatsFolder:FindFirstChild(fullBoatName) and boatsFolder[fullBoatName]:FindFirstChild("VehicleSeat") then
                            Tween(boatsFolder[fullBoatName].VehicleSeat.CFrame * CFrame.new(0,1,0))
                        end
                    end
                end
            end)
...
--// Attack Fish (Crew/Shark/Piranha) - NORMAL ATTACKS ONLY
...
spawn(function()
    while task.wait() do
        if AutoFarmFish then
            pcall(function()
                -- Only attack if no higher priority (Terrorshark)
                if not game.Workspace.Enemies:FindFirstChild("Terrorshark") then
                    local fish_types = {"Piranha", "Shark", "Fish Crew Member"}
                    for _, fish_name in ipairs(fish_types) do
                        if game.Workspace.Enemies:FindFirstChild(fish_name) then
                            for i,v in pairs (game.Workspace.Enemies:GetChildren()) do
                                if v.Name == fish_name then
                                    if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                                        repeat game:GetService("RunService").Heartbeat:wait()
                                            -- NO SKILLS FOR FISH - NORMAL ATTACKS ONLY
                                            EquipTool(SelectWeaponSeaFarm)
...
--// Attack Sea Beast - USE SKILLS ONLY
...
spawn(function()
    while task.wait() do
        if AutoFarmSeaBeast then
            pcall(function()
                -- Only attack if no higher priority (Terrorshark, Fish)
                if not game.Workspace.Enemies:FindFirstChild("Terrorshark") and not (game.Workspace.Enemies:FindFirstChild("Shark") or game.Workspace.Enemies:FindFirstChild("Piranha") or game.Workspace.Enemies:FindFirstChild("Fish Crew Member")) then
                        ...
                        for i,v in pairs (game.Workspace.SeaBeasts:GetChildren()) do
                            if v.Name == "SeaBeast1" then
                                if v:FindFirstChild("RootPart") and v.Health.Value > 0 then
                                    repeat game:GetService("RunService").Heartbeat:wait()
                                        SeaUseSkill = true -- ENABLE SKILLS FOR SEA BEAST
                                        -- No EquipTool here, handled in skill cycle
                                        ...
                                        -- NO AutoClick() - Using skills only
                                        game.Players.LocalPlayer.Character.Humanoid.Sit = false
...
--// Attack Ghost Boats - USE SKILLS ONLY
...
spawn(function()
    while task.wait() do
        if AutoFarmGhostBoats then
            pcall(function()
                -- Only attack if no higher priority (Terrorshark, Fish, SeaBeast)
                if not game.Workspace.Enemies:FindFirstChild("Terrorshark") and not (game.Workspace.Enemies:FindFirstChild("Shark") or game.Workspace.Enemies:FindFirstChild("Piranha") or game.Workspace.Enemies:FindFirstChild("Fish Crew Member")) and not game:GetService("Workspace").SeaBeasts:FindFirstChild("SeaBeast1") then
                        ...
                        for i,v in pairs (game.Workspace.Enemies:GetChildren()) do
                            if v.Name == "PirateBrigade" or v.Name == "PirateGrandBrigade" then
                                if v:FindFirstChild("Humanoid") and v:FindFirstChild("VehicleSeat") and v.Humanoid.Value > 0 then
                                    repeat game:GetService("RunService").Heartbeat:wait()
                                        SeaUseSkill = true -- ENABLE SKILLS FOR GHOST BOATS
                                        -- No EquipTool here, handled in skill cycle
...
--// Auto Leviathan (with skills)
Sea_Farm:addToggle('Auto Leviathan', AutoLeviathan, function(Value)
    AutoLeviathan = Value
    if AutoLeviathan then
        SelectBoat = "BeastHunter" -- Force BeastHunter for harpoon
    end
    CancelTween(AutoLeviathan)
end)
spawn(function()
    while task.wait() do
        if AutoLeviathan then
            pcall(function()
                -- Auto buy/summon Frozen Dimension if not in
                if not game.Workspace:FindFirstChild("FrozenDimension") then -- Assume detect Frozen
                    Tween(MysteriousScientistPos)
                    local hrp = game.Players.LocalPlayer.Character.HumanoidRootPart
                    if (MysteriousScientistPos.Position - hrp.Position).Magnitude <= 10 then
                        -- Talk NPC (use spy to confirm remote, assume CommF_)
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyItem", "Frozen Dimension") -- Spy to confirm
                    end
                    -- Then go rough sea and use item (assume equip and key)
                    local tool = game.Players.LocalPlayer.Backpack:FindFirstChild("Frozen Dimension") or game.Players.LocalPlayer.Character:FindFirstChild("Frozen Dimension")
                    if tool then
                        EquipTool("Frozen Dimension")
                        game:GetService("VirtualInputManager"):SendKeyEvent(true, "F", false, game) -- Assume activate with F, spy to confirm
                        wait(0.1)
                        game:GetService("VirtualInputManager"):SendKeyEvent(false, "F", false, game)
                    end
                end
                -- Detect Leviathan
                local levi = game.Workspace.Enemies:FindFirstChild("Leviathan")
                if levi then
                    local allSegmentsDead = true
                    for i, seg in pairs(levi:GetChildren()) do
                        if seg.Name:match("Segment") and seg:FindFirstChild("Humanoid") and seg.Humanoid.Health > 0 then -- Assume segment names
                            allSegmentsDead = false
                            repeat game:GetService("RunService").Heartbeat:wait()
                                SeaUseSkill = true -- USE SKILLS FOR LEVIATHAN
                                -- No EquipTool, handled in cycle
                                Tween(seg.HumanoidRootPart.CFrame * CFrame.new(0,50,0)) -- Attack blood tails
                                seg.HumanoidRootPart.CanCollide = false
                                seg.HumanoidRootPart.Size = Vector3.new(60,60,60)
                                seg.HumanoidRootPart.Transparency = 1
                                SeaMonName = seg.Name
                                SeaMonCFrame = seg.HumanoidRootPart.CFrame
                                SeaMonPosition = seg.HumanoidRootPart.Position
                                -- NO AutoClick() - Using skills only
                                game.Players.LocalPlayer.Character.Humanoid.Sit = false
                            until not AutoLeviathan or not seg.Parent or seg.Humanoid.Health <= 0
                            SeaUseSkill = false
                        end
                    end
                    if allSegmentsDead then
                        AutoHarpoon = true -- Auto toggle to harpoon
                    end
                else
                    if game.Players.LocalPlayer.Character:WaitForChild("Humanoid").Sit == false then
                        local boatsFolder = game:GetService("Workspace"):FindFirstChild("Boats")
...
--// Auto Harpoon Heart (after segments)
spawn(function()
    while task.wait() do
        if AutoHarpoon then
            pcall(function()
                -- Detect heart
                local heart = game.Workspace:FindFirstChild("LeviathanHeart") -- Assume name, spy to confirm
                if heart then
                    LeviathanHeartPos = heart.CFrame
                    local boatsFolder = game:GetService("Workspace"):FindFirstChild("Boats")
                    local boat = boatsFolder:FindFirstChild("BeastHunter")
                    if boat then
                        local harpoonSeat = boat:FindFirstChild("HarpoonSeat") -- Assume name
                        if harpoonSeat then
                            Tween(harpoonSeat.CFrame * CFrame.new(0,1,0))
                            game.Players.LocalPlayer.Character.Humanoid.Sit = true
                            TPB(heart.CFrame * CFrame.new(50, 20, 0)) -- Move boat near heart, higher y
                            if (heart.Position - harpoonSeat.Position).Magnitude <= 50 then
                                -- Auto fire harpoon (spy to find remote/args)
                                local harpoonRemote = boat:FindFirstChild("HarpoonRemote") -- Assume, spy to confirm
                                if harpoonRemote then
                                    harpoonRemote:FireServer(heart.Position) -- Assume arg is position
                                else
                                    -- Alternative: Simulate click
                                    game:GetService("VirtualInputManager"):SendMouseButtonEvent(0, 0, 0, true, game, 0) -- Left click
                                    wait(0.1)
                    ...
                    -- After done, auto sail back
                    local returnPos = (SelectReturnIsland == "Tiki" and TikiPos) or HydraPos
                    TPB(returnPos)
                    if (returnPos.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 10 then
                        AutoHarpoon = false
                        AutoSail = true -- Resume auto sea event
                    end
                end
            end)
        end
...
Config_Sea:addTextbox("Speed Boat Hack", SpeedAllBoat, function(Value)
    ...
    pcall(function()
        ...
        local fullBoatName = getFullBoatName()
        if boatsFolder and boatsFolder:FindFirstChild(fullBoatName) and boatsFolder[fullBoatName]:FindFirstChild("VehicleSeat") then
            boatsFolder[fullBoatName].VehicleSeat.MaxSpeed = tonumber(SpeedAllBoat) or SpeedAllBoat
        end
    end)
end)

-- Boat selection dropdown (NEW)
...
--// Skill use loop - FIXED VERSION (only cycles weapons when skills are actually needed)
spawn(function()
    while task.wait() do
        if SeaUseSkill then
            pcall(function()
                -- Only update position if we're actually using skills
                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if v.Name == SeaMonName or v.Name == "SeaBeast1" or v.Name == "PirateBrigade" or v.Name == "PirateGrandBrigade" then
                        SeaMonPosition = (v:FindFirstChild("HumanoidRootPart") and v.HumanoidRootPart.Position) or (v:FindFirstChild("VehicleSeat") and v.VehicleSeat.Position) or (v:FindFirstChild("RootPart") and v.RootPart.Position) or SeaMonPosition
                    end
                end
                for i,v in pairs(game:GetService("Workspace").SeaBeasts:GetChildren()) do
                    if v.Name == SeaMonName then
                        SeaMonPosition = v.RootPart.Position or SeaMonPosition
                    end
                end
                
                -- Only use ONE weapon type (the selected one) instead of cycling through all
                local tool_name = get_tool_name_by_type(SelectWeaponSeaFarm)
                if tool_name then
                    EquipTool(tool_name)
                    wait(0.2) -- Wait for equip
                    local playerChar = game:GetService("Players").LocalPlayer.Character
                    local tool = playerChar:FindFirstChild(tool_name)
                    if tool and tool:FindFirstChild("MousePos") then
                        tool.MousePos.Value = SeaMonPosition
                    end
                    
                    -- Use skills only if they are enabled in config
                    ...
                    wait(0.5) -- Wait after skills for next cycle
                end
            end)
        else
            -- When not using skills, just wait briefly to prevent high CPU usage
            wait(1)
        end
    end
end)
...
local WerewolfBoss = "Unbound Werewolf"
...
-- Function tìm và đánh monster của bản thân (NEW FUNCTION)
local function AttackMyMonster()
    pcall(function()
        if not CheckCharacter() then return false end
        ...
        -- Tìm monster của bản thân trong Workspace
        for _, monster in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
            if monster:FindFirstChild("Humanoid") and monster.Humanoid.Health > 0 and monster:FindFirstChild("HumanoidRootPart") then
                -- Kiểm tra xem monster có thuộc về người chơi hiện tại không
                if monster:FindFirstChild("PlayerOwner") then
                    local ownerValue = monster:FindFirstChild("PlayerOwner")
        ...
        if myMonster then
            print("🎃 Đã phát hiện monster của bản thân - Bắt đầu tấn công!")
            
            IsAttacking = true
            local monsterName = "MyMonster_" .. tostring(myMonster)
            LastHealth[monsterName] = myMonster.Humanoid.Health
            NoDamageTimer[monsterName] = 0
            
            local attackStart = tick()
            local lastMonsterPosition = myMonster.HumanoidRootPart.CFrame
            
            repeat
                task.wait(0.1)
                if not CheckCharacter() or not _G.AutoHalloweenEvent then break end
                
                -- Kiểm tra timeout (1 phút)
                if tick() - attackStart > 60 then
                    print("⏰ Timeout monster của bản thân - Chuyển target sau 1 phút")
                    break
                end
                
                -- Cập nhật vị trí quái
                if myMonster and myMonster:FindFirstChild("HumanoidRootPart") then
                    lastMonsterPosition = myMonster.HumanoidRootPart.CFrame
                end
                
                -- Chỉ Tween nếu quái còn sống và ở gần
                if myMonster and myMonster:FindFirstChild("Humanoid") and myMonster.Humanoid.Health > 0 then
                    EquipTool(SelectWeapon)
                    Tween(lastMonsterPosition * CFrame.new(0, 8, 0)) -- Đứng trên đầu quái
                    
                    -- Anti-leech và optimization
                    if myMonster:FindFirstChild("HumanoidRootPart") then
                        myMonster.HumanoidRootPart.CanCollide = false
                        myMonster.HumanoidRootPart.Size = Vector3.new(50,50,50)
                        myMonster.HumanoidRootPart.Transparency = 1
                    end
                    
                    AutoClick()
                    
                    -- Kiểm tra damage
                    if not CheckDamage(myMonster, monsterName) then
                        break
                    end
                end
                
            until not myMonster.Parent or not myMonster:FindFirstChild("Humanoid") or myMonster.Humanoid.Health <= 0
        ...
        if _G.AutoHalloweenEvent then
            pcall(function()
                ...
                if IsInRoom then
                    ...
                    -- ƯU TIÊN 2: Kiểm tra và đánh monster của bản thân (NEW PRIORITY)
                    if AttackMyMonster() then
                        IsInRoom = false
                        wait(2)
                        return
                    end
                    
                    -- ƯU TIÊN 3: Kiểm tra và đánh quái thường
                    ...
                    if #doors > 0 then
                        ...
                        if door and door.Parent and door:FindFirstChildWhichIsA("BasePart") then
                            ...
                            if UseHalloweenDoor(door) then
                                ...
                                -- SAU KHI VÀO PHÒNG: ƯU TIÊN TÌM VÀ ĐÁNH BOSS TRƯỚC
                                if AttackWerewolfBoss() then
                                    IsInRoom = false
                                    wait(2)
                                end
                                
                                -- Sau đó monster của bản thân
                                if AttackMyMonster() then
                                    IsInRoom = false
                                    wait(2)
                                end
...
-- THÊM VÀO PHẦN FARM_LEFT (Sau phần Farm Sea Event hoặc trước phần Boss Farm)

--// TYRANT OF THE SKIES FARM
local Tyrant_Farm = Farm_Left:addMenu('#Tyrant of the Skies Farm')

-- ensure global exists
if AutoTyrant == nil then AutoTyrant = false end

Tyrant_Farm:addToggle('Auto Farm Tyrant of the Skies', AutoTyrant, function(Value)
    AutoTyrant = Value
    CancelTween(AutoTyrant)
end)

-- Các mob cần farm trước khi summon/tiếp cận boss
local preFarmMobs = {
    "Sun-kissed Warrior [Lv. 2500]",
    "Isle Champion [Lv. 2525]", 
    "Serpent Hunter [Lv. 2550]",
...
-- Vị trí teleport khi tất cả eye về 0
local summonPosition = Vector3.new(-16264.6337890625, 152.9617043945312, 1390.417236328125)

-- Danh sách vị trí cần bắn
local shootPositions = {
    Vector3.new(-16255.2958984375, 153.0984649658203, 1458.849609375),
    Vector3.new(-16324.74609375, 163.4205322265625, 1442.236572265625),
    Vector3.new(-16291.046875, 158.2042999267578, 1467.3336181640625),
    Vector3.new(-16210.603515625, 151.88409423828125, 1463.1588134765625),
    Vector3.new(-16282.8017578125, 157.34503173828125, 1325.8197021484375),
    Vector3.new(-16329.9150390625, 152.95639038085938, 1321.531005859375),
    Vector3.new(-16213.1201171875, 157.62460327148438, 1330.4942626953125),
    Vector3.new(-16256.2724609375, 151.52520751953125, 1309.852783203125),
    Vector3.new(-16330.4560546875, 159.74081420898438, 1328.6685791015625),
    Vector3.new(-16305.7626953125, 155.0840606689453, 1463.6983642578125)
}

-- =======================
-- Helper: tìm Eye (đệ quy, case-insensitive)
-- =======================
local function FindEyePartByName(name)
    local lname = tostring(name):lower()
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj and obj:IsA("BasePart") and obj.Name:lower() == lname then
            return obj
        end
    end
    return nil
...
local function AreAllEyesZero()
    for i = 1, 4 do
        local eyeName = "Eye" .. i
        local eye = FindEyePartByName(eyeName)
        if not eye then
            return false
        end
...
-- =======================
-- Helper: robust load + equip Skull Guitar
-- =======================
local function EnsureSkullGuitarEquipped(timeoutSeconds)
    timeoutSeconds = timeoutSeconds or 6
    local Players = game:GetService("Players")
    local RepStorage = game:GetService("ReplicatedStorage")
    local player = Players.LocalPlayer
    if not player then return false, "No LocalPlayer" end
    
    -- 1) Try to call server to LoadItem
    local success, err = pcall(function()
        local remotes = RepStorage:WaitForChild("Remotes", 2)
        if remotes and remotes:FindFirstChild("CommF_") then
            remotes.CommF_:InvokeServer("LoadItem", "Skull Guitar", { "Guns" })
        else
            if remotes then
                for _, r in ipairs(remotes:GetChildren()) do
                    if r.Name:lower():find("comm") then
                        pcall(function() r:InvokeServer("LoadItem", "Skull Guitar", { "Guns" }) end)
                    end
                end
            end
        end
    ...
    if not success then
        warn("EnsureSkullGuitarEquipped: LoadItem invoke failed:", err)
    end
    
    -- 2) wait for the tool to appear in Backpack or Character
    local backpack = player:WaitForChild("Backpack", 2)
    local t0 = tick()
    local foundTool = nil
    local candidateNames = { "Skull Guitar", "Soul Guitar" }
    while tick() - t0 < timeoutSeconds do
        if backpack then
            for _, name in ipairs(candidateNames) do
                foundTool = backpack:FindFirstChild(name)
        ...
        if not foundTool and player.Character then
            for _, name in ipairs(candidateNames) do
                foundTool = player.Character:FindFirstChild(name)
                if foundTool then break end
            end
        end
        if foundTool then break end
    ...
    -- 4) Call EquipEvent on tool if exist
    if foundTool.Parent == char then
        if foundTool:FindFirstChild("EquipEvent") and typeof(foundTool.EquipEvent.FireServer) == "function" then
            pcall(function() foundTool.EquipEvent:FireServer(true) end)
            task.wait(0.08)
        end
        
        if foundTool:FindFirstChild("RemoteEvent") and typeof(foundTool.RemoteEvent.FireServer) == "function" then
            pcall(function() foundTool.RemoteEvent:FireServer("Equip", true) end)
        end
    end
    
    -- Final check
    if player.Character and (player.Character:FindFirstChild(foundTool.Name) or player.Character:FindFirstChildOfClass("Tool") == foundTool) then
        return true, "Equipped"
    else
        pcall(function() foundTool.Parent = player.Character end)
        task.wait(0.08)
        if player.Character and player.Character:FindFirstChild(foundTool.Name) then
            return true, "EquippedAfterForceParent"
        end
    end
    return false, "CouldNotEquip"
end

-- ===== Helpers an toàn cho Tween/Teleport =====
local TweenLock = false
local DEBUG_TYRANT = false
...
local function isValidCFrame(cf)
    ...
    local p = cf.Position
    return isValidVector3(p)
end

local function SafeTween(targetCFrame, maxDistance)
    if TweenLock then
        if DEBUG_TYRANT then warn("[Tyrant] SafeTween blocked by lock") end
        return false, "Locked"
    end
    if not isValidCFrame(targetCFrame) then
        if DEBUG_TYRANT then warn("[Tyrant] Invalid CFrame passed to SafeTween:", targetCFrame) end
        return false, "InvalidCFrame"
    end
    
    local plr = game:GetService("Players").LocalPlayer
    if not plr or not plr.Character or not plr.Character:FindFirstChild("HumanoidRootPart") then
        if DEBUG_TYRANT then warn("[Tyrant] No character HRP for SafeTween") end
        return false, "NoHRP"
    end
    ...
    if dist > maxDistance then
        if DEBUG_TYRANT then warn(("[Tyrant] Target too far (%.1f > %.1f) - ignoring"):format(dist, maxDistance)) end
        return false, "TooFar"
    end
    
    if targetPos.Y < -500 or targetPos.Y > 5000 then
        if DEBUG_TYRANT then warn("[Tyrant] Target Y out of bounds:", targetPos.Y) end
        return false, "BadY"
    end
    ...
    if not ok then
        if DEBUG_TYRANT then warn("[Tyrant] SafeTween error:", err) end
        return false, "Error"
    end
    
    if DEBUG_TYRANT then print("[Tyrant] SafeTween OK ->", tostring(targetCFrame.Position)) end
    return true, "OK"
end

local function Vec3ToCFrameSafe(v3)
    if isValidVector3(v3) then
        return CFrame.new(v3)
    end
    return nil
end

-- Spawn loop cho Auto Farm Tyrant of the Skies
spawn(function()
    while task.wait() do
        if AutoTyrant then
            pcall(function()
                local allEyesZero = AreAllEyesZero()
                
                if allEyesZero then
                    -- Teleport đến vị trí triệu hồi
                    local summonCF = Vec3ToCFrameSafe(summonPosition)
                    if summonCF then
                        SafeTween(summonCF, 3000)
                    else
                        warn("[Tyrant] summonPosition invalid, skip tween")
                    end
                    task.wait(2)
                    
                    -- Load + Equip Skull Guitar
                    local ok, status = EnsureSkullGuitarEquipped(6)
                    if not ok then
                        warn("Tyrant auto: Không equip Skull Guitar -", status)
                    else
                        -- Bắn vào tất cả vị trí
                        for _, pos in ipairs(shootPositions) do
                            if isValidVector3(pos) then
                                local args = { [1] = "TAP", [2] = pos }
                                local player = game:GetService("Players").LocalPlayer
                                local tool = player.Character and player.Character:FindFirstChild("Skull Guitar") or player.Backpack and player.Backpack:FindFirstChild("Skull Guitar")
                                
                                if tool and tool:FindFirstChild("RemoteEvent") and typeof(tool.RemoteEvent.FireServer) == "function" then
                                    pcall(function() tool.RemoteEvent:FireServer(unpack(args)) end)
                                else
                                    pcall(function()
                                        local rem = game:GetService("ReplicatedStorage"):WaitForChild("Remotes",2)
                                        if rem and rem:FindFirstChild("CommF_") and typeof(rem.CommF_.InvokeServer) == "function" then
                                            rem.CommF_:InvokeServer(unpack(args))
                                        else
                                            if player:FindFirstChild("RemoteEvent") and typeof(player.RemoteEvent.FireServer) == "function" then
                                                player.RemoteEvent:FireServer(unpack(args))
                                            end
                                        end
                                    end)
                                end
                    ...
                    -- Farm boss Tyrant of the Skies
                    _G.SelectMonster = "Tyrant of the Skies"
                    CheckLevel(_G.SelectMonster)
                    Tyrant_Farm_Name = Ms
                    Tyrant_Farm_CFrame = CFrameMon
                        ...
                        for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                                if v.Name == Ms then
                                    ...
                                    if v:FindFirstChild("HumanoidRootPart") and isValidCFrame(v.HumanoidRootPart.CFrame) then
                                        targetCf = v.HumanoidRootPart.CFrame * Farm_Mode
                                    end
                                    
                                    if targetCf then
                                        SafeTween(targetCf, 3500)
                                    else
                                        if DEBUG_TYRANT then warn("[Tyrant] boss targetCF invalid, skipping Tween") end
                                        Tween(CFrameMon)
                                    end
                                    ...
                                    repeat
                                        ...
                                        if tick() - lastChangeTime > 2 then
                                            if v and v.Parent and v:FindFirstChild("HumanoidRootPart") and isValidCFrame(v.HumanoidRootPart.CFrame) then
                                                SafeTween(v.HumanoidRootPart.CFrame * Farm_Mode, 3500)
                                            end
                                            lastChangeTime = tick()
                                        end
                                        ...
                                        v.HumanoidRootPart.Size = Vector3.new(60,60,60)
                                        v.HumanoidRootPart.Transparency = 1
                                    until not AutoTyrant or not v.Parent or v.Humanoid.Health <= 0 or not game:GetService("Workspace").Enemies:FindFirstChild(v.Name)
                                end
                            end
                        end
                    else
                        if isValidCFrame(CFrameMon) then
                            SafeTween(CFrameMon, 3500)
                        end
                    end
                else
                    -- Farm mob bình thường nếu chưa có eye nào về 0
                    for _, mobName in ipairs(preFarmMobs) do
                        _G.SelectMonster = mobName
                        CheckLevel(_G.SelectMonster)
                        Tyrant_Farm_Name = Ms
                        Tyrant_Farm_CFrame = CFrameMon
...
local HasCakeQuest2 = false
local LastQuestCheck = 0
local NPCPos = CFrame.new(-2077, 252, -12373) -- Vị trí NPC để nhận quest
spawn(function()
    while wait() do
...
-- Function kiểm tra và hủy quest khác
local function CheckAndAbandonOtherQuests()
    ...
    if Quest.Visible then
        ...
        -- Nếu đang có quest khác không phải Head Baker, hủy nó
...
-- Function để nhận quest CakeQuest2 với check hoàn thành và trạng thái đã nhận
local function GetCakeQuest2()
    local Quest = game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest
    local QuestTitleObj = Quest.Container.QuestTitle.Title
    local QuestTitle = QuestTitleObj.Text
    -- Kiểm tra nếu đã nhận quest từ trước
    if HasCakeQuest2 and Quest.Visible and string.find(QuestTitle, "Head Baker") then
        return true -- Không cần nhận lại
    end
    -- Check nếu quest đã hoàn thành
    local isQuestCompleted = false
    if Quest.Visible then
        local QuestProgress = Quest.Container.QuestTitle.Progress
        if QuestProgress then
            local progressText = QuestProgress.Text
            local current, required = string.match(progressText, "(%d+)/(%d+)")
            if current and required and tonumber(current) >= tonumber(required) then
                isQuestCompleted = true
            end
        end
    end
    if isQuestCompleted or not string.find(QuestTitle, "Head Baker") or not Quest.Visible then
        -- Hủy quest hiện tại nếu có
        if Quest.Visible then
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
            wait(0.5)
        end
        ...
        -- Nhận quest mới
        local result = game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StartQuest","CakeQuest2",2)
        if result then
            HasCakeQuest2 = true
            LastQuestCheck = tick()
            return true
...
spawn(function()
    while task.wait() do
        if AutoCakePrince then
            pcall(function()
                -- Tự động hủy quest khác nếu có
                CheckAndAbandonOtherQuests()
            
                -- Tự động nhận quest CakeQuest2 với check hoàn thành
                local hasQuest = GetCakeQuest2()
                            ...
                            for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
                                if AutoCakePrince and v.Name == "Cake Prince" and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                                    ...
                                    repeat
                                        ...
                                        -- Kiểm tra nếu máu không giảm
                                        if tick() - lastHealthCheck >= 1 then
                                            if v.Humanoid.Health >= lastHealth - 10 then -- Cho phép sai số 10 máu
                                                -- Bắt đầu đếm thời gian không giảm máu
                                                if noDamageStartTime == nil then
                                                    noDamageStartTime = tick()
                                                elseif tick() - noDamageStartTime >= 3 then -- 3 giây không giảm máu
                                                    shouldTween = true
                                                end
                                            else
                                                -- Reset thời gian nếu máu giảm
                                                noDamageStartTime = nil
                                                shouldTween = false
                                            end
                                        ...
                                        -- Nếu đang giảm máu thì không Tween, chỉ đánh
                                ...
                                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                                    if (v.Name == "Cookie Crafter" or v.Name == "Cake Guard" or v.Name == "Baking Staff" or v.Name == "Head Baker") and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                                        ...
                                        repeat
                                            ...
                                            -- Kiểm tra nếu máu không giảm
                                            if tick() - lastHealthCheck >= 1 then
                                                if v.Humanoid.Health >= lastHealth - 10 then 
    
                                                    if noDamageStartTime == nil then
                                                        noDamageStartTime = tick()
                                                    elseif tick() - noDamageStartTime >= 1.5 then 
                                                        shouldTween = true
                                                    end
                                                else
            ...
            -- Reset trạng thái quest khi tắt AutoCakePrince
            if HasCakeQuest2 and tick() - LastQuestCheck > 10 then
                HasCakeQuest2 = false
            end
        end
    end
